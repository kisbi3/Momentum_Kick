...
double T = 0.73767076;    //Temperature, GeV
double q_1 = 0.76211042;    //GeV
*/
double T = 0.73795214;    //Temperature, GeV
double q_1 = 0.761635131;    //GeV
...
/*
double xx = 1.3562688;
double yy = 0.48280412;
*/
double xx = 1.35738805;
double yy = 0.48186191;
...
int n_1_pt = 150;
double dptf_1 = double ((15.1-0.1)/n_1_pt);  //func1 pt 출력 범위 : 0.15~11
double ddptf_1 = dptf_1/n_1;
double ptf_1 = 0.1;
int check2_1 = 1;
double ptf_start = ptf_1;
// double arr[3]= {0.};
...
void func1(double ptq){
    ...
    for(k_1=1;k_1<=n_1_pt+1;k_1++){
        ...
        sum_1_alice = sum_1_alice*2*2/3;
        sum_1_cms = sum_1_cms*2*2/3;
    ...
    double ptf_1 = ptf_start;
    // double ptf_1 = 0.05;
    // double ptf_2 = dptf_1*n_1_pt/2;
    double ptf_2 = (15.1-0.1)/2.;
    double phif_1 = phif0_1;
    // double arr[3]= {0.};
    int k_2 = (n_1_pt+2)/2;

    for(k_1=1;k_1<=(n_1_pt)/2;k_1++){
...
void ptdis_correction(double ptq){
    double etaf_1, dist_1, sum_1_alice, sum_1_cms, sum_1j, ptf_11, etajet_1, etacms_1, sum_1_alice_czyam, sum_1_cms_czyam;
    int i_1, j_1, k_1, l_1, h_1;

    std::ofstream correction("pTdis_correction.csv");
    correction<<"pt,ALICE_Ridge,CMS_Ridge,ALICE_Jet,CMS_Jet\n";
    // double CZYAM[2][400]={0.};
    // double Yridge[2][400]={0.};
    ...
    phif_1 = phif0_1;
    for(j_1=1;j_1<=n_1+1;j_1++){
        sum_1_alice_czyam += sum_1_alice*dphif_1;
        sum_1_cms_czyam += sum_1_cms*dphif_1;
        phif_1 += dphif_1;
    }
    ...
    for(l=1;l<=n_1*5;l++){
        ...
        double phif_r2 = (phif0_1+lastphi)/2.;
        // phif_1 = phif0_1;
        std::thread t9(func9, arr11, ptf_1, phif_r1, ptq, ddptf_1);
        std::thread t10(func9, arr11, ptf_1, phif_r2, ptq, ddptf_1);
        //ddptf_1
        t9.join();
        t10.join();
        ptf_1 += ddptf_1;
    ...
    correction<<1.25<<","<<arr11[0]-sum_1_alice_czyam<<","<<arr11[1]<<","<<sum_1j<<std::endl;
    correction.close();
}
...
void func2(double ptf_st, double ptf_end, double etaf_st, double etaf_end, double etacms_st, double etacms_end, double etaatlas_st, double etaatlas_end, double phif_st, double phif_end, int n, int check2, double ptjetcut, double q){
    ...
    if(ptjetcut == 0.){
        ...
        for(k=1;k<=n+1;k++){
            ...
            // std::cout<<phif<<std::setw(20)<<sum_alice<<std::setw(20)<<sum_cms<<std::setw(20)<<sum_atlas<<std::endl;
            // 2를 곱하는 것은 eta가 +인 영역과 -인 영역을 모두 고려해야 하기 때문!
            // 왜인지는 모르겠지만 sum *= 2*2/3이 안곱해짐!
            sum_alice = sum_alice*2*2/3;
            sum_cms = sum_cms*2*2/3;
            sum_atlas = sum_atlas*2*2/3;

            norm += sum_atlas*dphif;

            // std::cout<<phif<<std::setw(20)<<sum_alice*2*2/3<<std::setw(20)<<sum_cms<<std::setw(20)<<sum_atlas<<std::endl;
...
int main()
{
    ...
    // //상대오차를 구하기 위한 계산
    // std::vector<std::string> csv_read_row(istream &file, char delimiter);
    // std::ifstream file("/home/jaesung/OneDrive/Code/WongCode/13TeV-Alice/HEPData-ins1840098-v1-csv/1-NTRIGDN-DPHI.csv");
    ...
    // thread t9(ptdis_correction, q_1);
    // t9.join();
    // std::cout<<"Correction Calculate end"<<std::endl<<std::endl<<std::endl;
    ...
    thread t2(func2, 1., 2., 1.6, 1.8, 2., 4., 2., 5., -1.18, 1.18, 300, 1, 0., q_1);   //1D phi correlation, pt = 1~2
    thread t3(func2, 2., 3., 1.6, 1.8, 2., 4., 2., 5., -1.18, 1.18, 300, 1, 0., q_1);   //1D phi correlation, pt = 2~3
    thread t4(func2, 3., 4., 1.6, 1.8, 2., 4., 2., 5., -1.18, 1.18, 300, 1, 0., q_1);   //1D phi correlation, pt = 3~4
    thread t5(func2, 1., 4., 1.6, 1.8, 2., 4., 2., 5., -1.18, 1.18, 300, 1, 0., q_1);   //1D phi correlation, pt = 1~4
    // q에 multiplicity dependence가 없는 경우
    thread t8(func2, .5, 5., 1.6, 1.8, 2., 4., 2., 5., -1.18, 1.18, 300, 1, 0., q_1);   //1D phi correlation, pt = 0.5~5
