...
double Nk_multi_1 = 3.5;
double Nk_multi_2 = 180.;
...
void func10(double arr[], double ptf, double phif, double etaf, double etacms, double etajet, double qq, double dptff, double Multi){
    ...
    for(i=1;i<=n_1/2;i++){
        arr[0] += Nk_multi(Multi)*frnk(ptf)*ptf*RidgeDis(Aridge, ptf, etaf, phif, check2_1, qq, T)*detaf_1*dphif_1*dptff/delta_Deltaeta;
...
void func9(double arr[], double ptf_1, double phif_1, double q, double dptfff, double Multi){
    ...
    for(j_9=1;j_9<=n_1/2;j_9++){
        ...
        etacms_1 = etacms0_1;
        etajet_1 = etajet0_1;
        std::thread t11(func10,arr, ptf_1, phif_1, etaf_1, etacms_1, etajet, q, dptfff, Multi);
        std::thread t12(func10,arr, ptf_1, phif_1, (etaf_1+lasteta)/2., (etacms_1+lastetacms)/2., (etajet_1+lastetajet)/2., q, dptfff, Multi);
...
void func11(double ptf2, double phif, int k, double q, char dist, double dptf2, double Multi){
    ...
    if(dist == 'r'){
        ...
        std::cout<<"Ridge"<<std::setw(20)<<Multi<<std::setw(20)<<ptf2<<std::setw(20)<<k<<std::setw(20)<<arr1[0]<<std::setw(20)<<arr1[1]<<std::endl;
...
void xaxis_multi(double ptq, double Aridge_multi){
    ...
    std::ofstream foutmulti("./phiCorr_Nk/Multiplciity_dependence.csv");
    ...
    double ptf_1 = 0.5;
    for(int jj=0;jj<(200);jj=jj+5){
        Multiplicity = jj;
        etaatlas = 2.;
        ptf_1 = 1.;
        detect_arr[2] = 0.;
        atlas(detect_arr, ptf_1, Aridge_multi, etaatlas, -1.18, 1, ptq, T, dptf, detaatlas, 4., n, Multiplicity);
        detect_arr[2] *= 2*2/3;
        CZYAM[0][jj]= detect_arr[2]*2.36;
    ...
    for(int jj=0;jj<(200);jj=jj+5){
        ...
        for(int ii=1;ii<n_1+1;ii++){
            etaatlas = 2.;
            ptf_1 = 1.;
            phif_1 = (double(2.36/n_1))*double(ii)-1.18;
            detect_arr[2] = 0.;
            atlas(detect_arr, ptf_1, Aridge_multi, etaatlas, phif_1, 1, ptq, T, dptf, detaatlas, 4., n, Multiplicity);
            // detect_arr[2] *= 2*(2/3)*(2.56/n_1);
            // disttt += detect_arr[2]*2*(2/3);
            Yridge[0][jj] += detect_arr[2]*(double(2.36/n_1));
...
int main()
{
    ...
    std::ofstream fout("./phiCorr_Nk/FittingParameters_Ridge.csv");
    ...
    // thread t2(func2, 1., 2., 1.6, 1.8, 2., 4., 2., 5., -1.18, 1.18, 300, 1, 0., q_1, 0.);   //1D phi correlation, pt = 1~2
    // thread t3(func2, 2., 3., 1.6, 1.8, 2., 4., 2., 5., -1.18, 1.18, 300, 1, 0., q_1, 0.);   //1D phi correlation, pt = 2~3
    // thread t4(func2, 3., 4., 1.6, 1.8, 2., 4., 2., 5., -1.18, 1.18, 300, 1, 0., q_1, 0.);   //1D phi correlation, pt = 3~4
    // thread t5(func2, 1., 4., 1.6, 1.8, 2., 4., 2., 5., -1.18, 1.18, 300, 1, 0., q_1, 0.);   //1D phi correlation, pt = 1~4
// double Aridge_multi, double ptf_st, double ptf_end, double etaf_st, double etaf_end, double etacms_st, double etacms_end, double etaatlas_st, double etaatlas_end, double phif_st, double phif_end, int n, int check2, double ptjetcut, double q, double Multi){
    ...
    thread t22(xaxis_multi, q_1, Aridge_q[ 4]);

    // thread t21(func2, Aridge_q[ 0], .5, 5., 1.6, 1.8, 2., 4., 2., 5., -1.18, 1.18, 300, 1, 0., q_1,  95);
    // thread t01(func2, Aridge_q[ 0], .5, 5., 1.6, 1.8, 2., 4., 2., 5., -1.18, 1.18, 300, 1, 0., q_1, 105);   //1D phi correlation, pt = 0.5~5 q = 0.80
    // thread t02(func2, Aridge_q[ 1], .5, 5., 1.6, 1.8, 2., 4., 2., 5., -1.18, 1.18, 300, 1, 0., q_1, 115);   //1D phi correlation, pt = 0.5~5 q = 0.85
    // thread t03(func2, Aridge_q[ 2], .5, 5., 1.6, 1.8, 2., 4., 2., 5., -1.18, 1.18, 300, 1, 0., q_1, 125);   //1D phi correlation, pt = 0.5~5 q = 0.90
    // thread t04(func2, Aridge_q[ 3], .5, 5., 1.6, 1.8, 2., 4., 2., 5., -1.18, 1.18, 300, 1, 0., q_1, 135);   //1D phi correlation, pt = 0.5~5 q = 0.95
    // t01.join();
    // t02.join();
    // t03.join();
    // t04.join();
    ...
    // thread t6(func2, 1., 2., 1.6, 1.8, 2., 4., 2., 5., -1.18, 1.18, 300, 1, 10., q_1, 0.);   //1D phi correlation, pt = 1~2 (jet event cut)
    // thread t7(func2, 1., 2., 1.6, 1.8, 2., 4., 2., 5., -1.18, 1.18, 300, 1, 20., q_1, 0.);   //1D phi correlation, pt = 1~2 (jet event cut)
