...
def Nk_phib(phis_host, b_scalar_host):   # scalar, vector
    beta, l_scalar = cp.meshgrid(cp.linspace(0., cp.pi/2, nbeta), cp.linspace(0., .8, nl_step))
    b0_scalar_gpu = cp.linspace(0., .8, nb0)
    ...
    for b0 in b0_scalar_gpu:
        ...
        Nk_dist2 = cp.sum((top+bot)*dbeta*dl/2)
        # print("\n b0 : ", b0, "\n Nk : ", Nk_dist2)
...
def Nk_b0phib(b0_scalar, beta, scalar_l, phis, b):   #scalar, scalar, scalar
    ...
    # return step_bpA*step_bpB*SIGMA*KP*(TT_scalar(b0_scalar, phis, beta, scalar_l, b))/(2*(T0+scalar_l))
...
# def TT_scalar(b0, phis, beta, l, b):
#     squareA = cp.pow(b0,2)+cp.pow(l,2)+cp.pow(b,2)/4.-b*l*cp.cos(phis)-b*b0*cp.cos(beta)+2.*l*b0*(cp.cos(beta)*cp.cos(phis)+cp.sin(beta)*cp.sin(phis))
#     squareB = cp.pow(b0,2)+cp.pow(l,2)+cp.pow(b,2)/4.+b*l*cp.cos(phis)+b*b0*cp.cos(beta)+2.*l*b0*(cp.cos(beta)*cp.cos(phis)+cp.sin(beta)*cp.sin(phis))
#     TA_p = (3./(2.*cp.pi*cp.pow(RA,3)))*cp.sqrt(cp.pow(RA,2)-squareA)
#     TB_p = (3./(2.*cp.pi*cp.pow(RB,3)))*cp.sqrt(cp.pow(RB,2)-squareB)
#     if(RA - cp.sqrt(squareA) > 0. and RB -  sqrt(squareB) > 0.)
#     {return(TA_p + TB_p);}
#     else return(0.)
