...
cc   To compile:   f77 -o momkick02.x momkick02.f 
      ...
      open ( 5,file='momkick02.05',status='unknown')
      open ( 6,file='momkick02.06',status='unknown') 
      open ( 7,file='momkick02.07',status='unknown') !plt dN/ptdpt,dN/dpt,etc
      open ( 8,file='momkick02.08',status='unknown') !plt dN/dfi
      open ( 9,file='momkick02.09',status='unknown') !plt dN/deta(eta),R,J,J+R
      open (10,file='momkick02.10',status='unknown') !prepr dF(fi,eta)4 mathca 
...
c      calcualte acceptance correcetion to be used when (izetaCase=1 and iacccor=1)
 ...
 40    read (5,*)   ptmin, dpt, npt ! final particle pt 
       ...
       read (5,*)  iptplt,ifiplt,ietaplt,ifietaplt
                  ...
                  ! if ifizetaplt=2 plot dn/dphidets(phi,zeta), zeta=x, fi=y
...
cc     plot  fi=x, zeta=y, yield=z
         ...
         do izeta=1,nzeta
            write (10,*) (fRNkch*Peta(izeta,ifi,1),ifi=nfi,2,-1),
     >                  (fRNkch*Peta(izeta,ifi,1),ifi=1,nfi)   ! at pt=pt(1)
         enddo                ! fi is x, and zeta is y.

         go to 170

cc     plot  zeta=x, fi=y, yield=z
 165        do ifi=-nfi+2,nfi
            if (ifi .le. 0)  ifiabs=iabs(ifi)+2
            if (ifi .gt. 0)  ifiabs=ifi
           write (10,*) (fRNkch*Peta(izeta,ifiabs,1),izeta=1,nzeta)    
                              ! at pt=pt(1) location
